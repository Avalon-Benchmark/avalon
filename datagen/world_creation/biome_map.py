...
from datagen.world_creation.heightmap import FloatListNP
...
from datagen.world_creation.heightmap import MapIntNP
from datagen.world_creation.heightmap import Point2DListNP
from datagen.world_creation.heightmap import Point3DListNP
from datagen.world_creation.heightmap import SpecialBiomes
from datagen.world_creation.heightmap import create_kd_tree
from datagen.world_creation.heightmap import get_border_points
...
from datagen.world_creation.utils import print_biome_legend
...
def hex_to_rgb(value: str) -> Tuple[float, float, float]:
    ...
    return tuple([srgb2lin(x) for x in srgb_color])
...
#     a = vec3(0.055)
#     return mix(
#         (vec3(1.0) + a) * pow(color.rgb, vec3(1.0 / 2.4)) - a,
#         12.92 * color.rgb,
...
class SceneryConfig:
    ...
    border_mode: BorderMode = BorderMode.LINEAR
...
class BiomeHeightMap:
    ...
    biomes: Tuple[Biome, ...]
    biome_id: MapIntNP
    # TODO: not clear that this should be cached and copied...
    moisture: MapFloatNP
    ...
    # TODO: actually, will need to be careful about staying away from buildings
    def get_random_points_in_biome(
        ...
        if weights is None:
            ...
            if config.border_mode == BorderMode.HARD:
                weights[weights < config.border_distance] = 0.0
                weights[weights > 0.0] = 1.0
            elif config.border_mode == BorderMode.LINEAR:
                weights = np.clip(weights / config.border_distance, 0, 1)
            elif config.border_mode == BorderMode.SQUARED:
...
def make_fast_biome(map: HeightMap, natural_biome_config: BiomeConfig):
    ...
    return BiomeHeightMap(map, biomes, biome_id, moisture, border_distances, config)
...
# TODO: rescale elevation and moisture per config
def make_biome(
    map: HeightMap,
    config: BiomeConfig,
    rand: np.random.Generator,
    ...
    # TODO: non-linear and noised transforms of distance -> moisture
    ...
    biome_map = BiomeHeightMap(map, config.biomes, biome_id, moisture, border_distances, config)
