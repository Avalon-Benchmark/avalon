...
from datagen.world_creation.utils import plot_triangulation
...
def calculate_normals_for_faces_and_vertices(vertices: np.ndarray, faces: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    norm[faces[:, 1]] += n
    norm[faces[:, 2]] += n
    # TODO: this call ends up having some zero length normals :(
    # normalize_v3(norm)
...
class Terrain:
    def __init__(
        ...
        self.fine_triangulation = Triangulation(
            biome_map,
            is_climbable,
            point_density_in_points_per_square_meter,
            rand,
        ...
        self.coarse_triangulation = Triangulation(
...
# TODO: might want to make per-vertex colors for the coarse world
def _get_export_data_inner(
    global_triangle_indices: np.ndarray, triangulation: Triangulation
) -> Union[None, Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]]:
    ...
    # TODO: very inefficient. Exports each triangle separately
...
def _create_terrain_mesh_node(
    ...
    # TODO: this whole thing is extremely inefficient:
    #  1. We really should be only printing out the indices that are actually required for each surface,
    #  which requires remapping the indices appropriately
    #  2. We should not be printing out such high precision floats
    #  3. all of the junk in this function should be appropriately vectorized
...
def _create_static_body(index: int, scene: ImprovedGodotScene, triangles: np.ndarray, vertices: np.ndarray) -> GDNode:

    # TODO: make this whole thing less horrifically inefficient
