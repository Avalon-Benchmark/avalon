...
class WorldConfig:
    ...
    seed: int
    # controls how many "tiles" will be exported for this world. Serves as a simple LOD (level of detail) parameter
    # that allows us to run much larger worlds even on low-resource devices like the oculus
    # TODO: this parameter should really be moved somewhere else, as it only affects the export, not the actual geometry
    # TODO: re-enable non-square worlds. Most logic supports them, but in the scramble to finish everything by the deadline
    #  we disabled them, so it's likely that they no longer work properly. For now just set them to the same thing.
    x_tile_count: int
    z_tile_count: int
...
class BuildingConfig:
    ...
    obstacle_builder: Optional[ObstacleBuilder] = None
