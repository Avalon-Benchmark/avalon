...
from datagen.world_creation.tasks.constants import IS_SHORE_COMPUTED
...
def uniform_fraction_variation(rand: np.random.Generator, variation: float):
    """:returns: a float in the raange (1.0-variation, 1.0+variation)"""
    return rand.uniform(1.0 - variation, 1.0 + variation)


def calculate_fractal_iteration_count(
...
def power_interpolation(value: float, min_value: float, max_value: float, power: float) -> float:
    inv_power = 1.0 / power
    range_value = max_value ** inv_power - min_value ** inv_power
    return min_value + (range_value * value) ** power


def range_via_difficulty(min_val: float, max_val: float, difficulty: float, rand: np.random.Generator) -> float:
    delta = max_val - min_val
    return min_val + delta * rand.uniform() * difficulty


def world_2d_to_3d(point_2d: Point2DNP, world: NewWorld) -> Point3DNP:
...
def create_world_from_constraint(
    ...
    if constraint is None:
        ...
        locations = world.begin_height_obstacles(locations)
    else:
...
def create_world_for_skill_scenario(
    ...
    for retry_num in range(MAX_OVERALL_RETRIES):
        ...
        fractal_iteration_count, size_in_meters = calculate_fractal_iteration_count(
        ...
        world = NewWorld.build(config, export_config, biome_config=biome_config, is_debug_graph_printing_enabled=False)
        if IS_SHORE_COMPUTED:
            assert biome_config is not None
            world.special_biomes = world.map.erode_shores(rand, biome_config)
        ...
        for ideal_shore_dist in ideal_shore_dist_options:
            ...
            for i in range(MAX_SPAWN_TRIES_PER_MAP):
                ...
                spawn_point = world_2d_to_3d(spawn_point_2d, world) + HALF_AGENT_HEIGHT_VECTOR * 1.1
                ...
                for food_point_2d in food_point_2ds:
                    ...
                    if visibility_calculator is None:
                        ...
                        is_food_visible = visibility_calculator.is_visible_from(
                            ...
                            world_2d_to_3d(food_point_2d, world) + visibility_offset,
                        )
                    if is_food_visible:
                        ...
                        if final_distance < object_radius * 2.0:
                            world.flatten((spawn_point_2d + food_point_2d) / 2.0, flatten_radius, object_radius * 3.0)
                        else:
                            flatten_radius = min([flatten_radius, final_distance / 2.0])
                            world.flatten(spawn_point_2d, flatten_radius, object_radius)
                            world.flatten(food_point_2d, flatten_radius, object_radius)
                        if is_logging:
                            logger.info(f"Visibility checks: {num_visibility_check}")
                        return world, WorldLocationData(
                            island=island_mask_without_fresh_water,
                            spawn=spawn_point,
                            goal=world_2d_to_3d(food_point_2d, world) + food_offset,
                        )
                    else:
                        if is_debugging:
...
def generate_world_config(
    ...
    if is_platonic_world:
        ...
        mountain_radius = range_via_difficulty(0.6, 0.4, diversity, rand)
        mountain_noise_scale = range_via_difficulty(5.0, 3.0, diversity, rand)
        mountain_offset = range_via_difficulty(0.45, 0.3, diversity, rand)
        ...
        if iterated_noise_scale > 15:
            ...
            smooth_min = 1.0
        if is_smoothed:
            blur_meters_std_dev = range_via_difficulty(1.5, smooth_min, diversity, rand)
        height_ratio = range_via_difficulty(0.05, 0.2, diversity, rand)
    else:
        ...
        if world_type == WorldType.JAGGED:
            ...
            if iterated_noise_scale > 0.1:
                blur_meters_std_dev = range_via_difficulty(1.0, 0.0, diversity, rand)
            ...
            if iterated_noise_scale > 0.04:
                blur_meters_std_dev = range_via_difficulty(2.0, 0.2, diversity, rand)
            ...
            if mountain_mode == 0:
                ...
                mountain_offset = range_via_difficulty(0.45, 0.35, diversity, rand)
                ...
                mountain_radius_decay = 1.0
                mountain_radius = range_via_difficulty(0.6, 0.2, diversity, rand)
                ...
                mountain_offset = range_via_difficulty(0.45, 0.2, diversity, rand)
            else:
    ...
    tile_count = 1
    # our tiling logic is basically--only bother exporting with tiles if the world is sufficiently large and it is enabled
    if export_config is not None and export_config.num_tiles:
        if export_config.num_tiles > 1 and size_in_meters > 100.0:
            tile_count = export_config.num_tiles
    # just for debugging:
